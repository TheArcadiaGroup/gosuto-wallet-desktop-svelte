...
<div
	class="w-full rounded-2xl shadow-md flex flex-col px-8 py-6 gap-2 max-w-2xl dark:bg-dark-grey dark:text-light-gray"
>
	<div class="font-bold dark:font-semibold dark:text-white">
		{stake?.name || 'unknown wallet name'}
	</div>
	<div class="grid grid-cols-3 w-full place-content-between text-sm">
	...
	<div class="grid grid-cols-2 w-full place-items-center text-sm">
		<div
			class="border-r border-b w-full text-center border-dashed border-light-grey pb-2 {stake?.elapsedSeconds ===
				0 && 'text-highlight'}"
		>
			{formatTime(stake?.elapsedSeconds || 0)}
		</div>
		<div
			class="border-b w-full text-center border-dashed border-light-grey pb-2 {stake?.fullSeconds ===
				0 && 'text-highlight'}"
		>
	...
	<div class="w-full text-sm mt-2">
		<div class="w-full text-center">Total time until reward</div>
		<div
			class="w-full text-center {stake?.fullSeconds - stake?.elapsedSeconds === 0 &&
				'text-highlight'}"
		>
