...
const { casperDelegationContractHexCode } = './constants';

const TEST_NET_RPC_API = 'http://52.70.214.247:7777/rpc';
const MAIN_NET_RPC_API = 'http://52.70.214.247:7777/rpc';
...
module.exports = {
	...
	// Delegate stake
	delegate: async ({ privateKey, validatorPublicKey, amount, network = 'casper-test' }) => {
		const client = new CasperClient(network === 'casper' ? MAIN_NET_RPC_API : TEST_NET_RPC_API);
		const publicKey = Keys.Ed25519.privateToPublicKey(privateKey);
		const keyPair = Keys.Ed25519.parseKeyPair(publicKey, privateKey);
		const deployParams = new DeployUtil.DeployParams(keyPair.publicKey, network);
	...
	// Undelegate
	undelegate: async ({ privateKey, validatorPublicKey, amount, network = 'casper-test' }) => {
		const client = new CasperClient(network === 'casper' ? MAIN_NET_RPC_API : TEST_NET_RPC_API);
		const publicKey = Keys.Ed25519.privateToPublicKey(privateKey);
		const keyPair = Keys.Ed25519.parseKeyPair(publicKey, privateKey);
		const deployParams = new DeployUtil.DeployParams(keyPair.publicKey, network);
