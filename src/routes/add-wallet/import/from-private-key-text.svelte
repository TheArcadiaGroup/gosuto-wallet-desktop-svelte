<script lang="ts">
	...
	const checkPrivateKey = () => {
		...
		} | null = accDetailsFromPvk(privateKey);
		try {
	...
	const accDetailsFromPvk = (pvk: string) => {
		try {
			const { Keys } = window.CasperSDK;

			const pvkBuffer = Buffer.from(pvk, 'hex');
			let keyPair: any;
			let algorithm: 'ed25519' | 'secp256k1';

			// Secp256k1
			if (pvkBuffer.length === 32) {
				const pvkUint8 = Keys.Secp256K1.parsePrivateKey(Buffer.from(pvk, 'hex'), 'raw');
				const pbkUint8 = Keys.Secp256K1.privateToPublicKey(pvkUint8);

				keyPair = Keys.Secp256K1.parseKeyPair(pbkUint8, pvkUint8, 'raw');
				algorithm = 'secp256k1';
			} else {
				const pvkUint8 = Keys.Ed25519.parsePrivateKey(Buffer.from(pvk, 'hex'));
...
<div class="fileImport-wrapper">
	<div class="fileImport-content">
		...
		<div class="fileImport-input-wrapper">
			<label class="fileImport-label" for="private_key">Private Key</label>
		...
		{/if}
		{#if privateKeyValid && !walletExists && accountHash && accountHex && privateKey}
			<div class="success-div text-center leading-5">Private Key Verified</div>
		{/if}
	</div>
